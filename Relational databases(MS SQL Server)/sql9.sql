use master;
go
if DB_ID (N'lab99') is not null
drop database lab99;
go
CREATE DATABASE lab99
ON PRIMARY
(
    NAME = lab99primary,
    FILENAME = '/var/opt/mssql/data/lab99primary.mdf',
    SIZE = 10,
    MAXSIZE = UNLIMITED,
    FILEGROWTH = 5
),
(
    NAME = lab99secondary,
    FILENAME = '/var/opt/mssql/data/lab99secondary.ndf',
    SIZE = 10,
    MAXSIZE = UNLIMITED,
    FILEGROWTH = 5
)
...
(
    NAME = lab99log,
    FILENAME = '/var/opt/mssql/data/lab99log.ldf',
    SIZE = 5,
    MAXSIZE = 20,
    FILEGROWTH = 5
);
...
if OBJECT_ID(N'Uniq_Book',N'UQ') IS NOT NULL
	ALTER TABLE Book DROP CONSTRAINT Uniq_book
go
