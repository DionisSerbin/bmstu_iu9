...
public class init {
    ...
    private void generateDFGlobal() {
        ...
        postOrder.forEach(x -> {
            ...
            x.succs.forEach(succ -> {
                if (x != succ.immediateDom) {
                    ...
                    System.out.println("добавлен предок " + succ.name);
                }
            });
                ...
                if (null != dominanceFrontier.get(child)) {
                    dominanceFrontier.get(child).forEach(y -> {
                        if (x != y.immediateDom) {
                            ...
                            System.out.println("добавлен потомок " + y.name);
                        }
                    });
                }
            });
        ...
        System.out.println("Доминаторы:");
        for (Vertex v : postOrder) {
            Set<Vertex> vSet = dominanceFrontier.get(v);
            String s = "";
            for (Vertex x : vSet)
                s += x.name.toUpperCase() + " ";
            System.out.println("вершина: " + v.name.toUpperCase() + ": [" + s + "]");
        }
        System.out.println();
    }
    ...
    private Set<Vertex> generateDFSet(Set<Vertex> vertexSet) {
        ...
        System.out.println("Множество вершин на фронте для [" + src + "]:");
        String s = "";
        for (Vertex x : res)
            s += x.name.toUpperCase() + " ";
    ...
    private void placePhi() {
        ...
        for (Var k : varToUsageVertices.keySet()) {
            ...
            for (Vertex x : phiSet) {
                ...
                System.out.println("добавленные фи");
    ...
    private void traverse(Vertex v, Var p) {
        ...
        for (Stmt stmt : v.statements) {
            System.out.println("  выражение: " + stmt);

            if (!stmt.isPhi) {
                System.out.print("    правое значение: " + stmt + " \t-> ");
            ...
            if (stmt.isAss && stmt.lhs.equals(p)) {
                System.out.print("    левое значение: " + stmt + " \t-> ");
    ...
    public static void main(String args[]) throws IOException {
        init CFG = new init();
        switch (args[0]){
            case "1": CFG.buildCFGSample1(); break;
            case "2": CFG.buildCFGSample2(); break;
            case "3": CFG.buildCFGSample3(); break;
            case "4": CFG.buildCFGSample4(); break;
            default:
                break;
        }
        ...
        switch (args[0]){
            case "1": CFG.printGraph(1); break;
            case "2": CFG.printGraph(2); break;
            case "3": CFG.printGraph(3); break;
            case "4": CFG.printGraph(4); break;
            default:
                break;
        }
        CFG.printBaseBlocks();
    ...
    private void printDot(init CFG) throws IOException {
        File dotfile = new File("graph.dot");
        Files.write(dotfile.toPath(), CFG.toDot().getBytes());
    }

    private String toDot() {
        StringBuilder str = new StringBuilder("digraph {\n");
        dominanceFrontier.keySet().forEach(x -> {
            for(Vertex y : x.succs){
                if(y.precs.contains(x)){
                    str.append("[label= \"").append(x.toString().replaceAll("\n", " ")).append("\"]").append("->").append("[label= \"").append(y.toString().replaceAll("\n", " ")).append("\"]").append("\n");
                }
            }
        });
        str.append("}\n");
