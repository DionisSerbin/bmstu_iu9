...
;<Body>     ::= if <Body> endif <Body> | word <Body> | .
;word       ::= любая лексема, кроме if endif define end .
...
(define (parse xs)
  ...
  (define (body xs)
    (cond
      ...
      ((equal? (vector-ref xs place) 'if)
       (begin
         (set! place (+ place 1))
         (call/cc
          (lambda (memo)
            (let loop ((plc place) (countif 1))
              (cond
                ((>= plc (vector-length xs)) (memo '(#f)))
                ((equal? (vector-ref xs plc) 'if) (loop  (+ plc 1) (+ countif 1)))
                ((not (equal? (vector-ref xs plc) 'endif)) (loop (+ plc 1) countif))
                ((> countif 1) (loop (+ plc 1) (- countif 1)))
                ((= countif 1) (memo (cons (list 'if (body xs)) (body xs))))
                ((< countif 1) (memo '(#f)))))))))
      ((equal? (vector-ref xs place) 'endif)
       (begin
         (set! place (+ place 1))
         '()))
      ((or (number? (vector-ref xs place)) (symbol? (vector-ref xs place)))
       (begin
         (set! place (+ place 1))
